...
class EmailConfirm(View):
    ...
    def get(self, request, code=None):
        user_id = cache.delete(f'conf_{code}')
        if user_id and (user := User.objects.filter(id=user_id)):  # Ищем по ключу id пользователя
            user = user.first()  # Ищем пользователя по id
...
class LinkResetPassword(View):
    ...
    def get(self, request, code=None):
        ...
        if user_id and (user := User.objects.filter(id=user_id)):
            user = user.first()
    ...
    def post(self, request, code=None):
        ...
        if form.is_valid():
            ...
            cache.delete(f'wrong_{user.email}')  # Сброс счетчика неверных попыток после успешного сброса пароля
